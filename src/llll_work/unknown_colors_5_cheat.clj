(ns llll-work.unknown-colors-5-cheat
...
(defsound happy option-d
  (let [trig (impulse (u/dt:kr 2 [4 12 32]))
        freq (* dr (u/dq trig (range 1000 10000 300)))
        snd (* (sin-osc (u/rg-lin (lf-pulse 8) (max 300 (/ freq 4)) freq))
               (env-gen (env-perc 0.5 0) trig))]
    (comb-n snd 1/4 1/4 1)))

(defsound happy option
  (splay (map #(let [trig (impulse (u/dt:kr 2 [4 12 32]) %)
                     freq (u/dq trig (range 1000 10000 300))
                     snd (* (sin-osc (u/rg-lin (lf-pulse 8) (max 300 (/ freq 4)) freq)) 8
                            (env-gen (env-perc 0.5 0) trig))]
                 (comb-n snd 1/4 1/4 2))
              (u/n-range 0 1 16))))
...
(defsound delight option
  (let [trig (impulse 8)
        freq (u/dq trig (flatten [(u/n-range 400 1200 8)
                                  (u/n-range 2000 2300 4)]))
  ...
  (let [trig (impulse 8)
    ...
    (-> (* filter env)
        ...
        (u/reduce-> (fn [acc x] (+ acc (comb-l acc 1/8 1/8 x)))
                    [0.2 0.5 0.2])
        (* (saw (!! (if b0 20 50))))
        (u/reduce-> (fn [acc x] (+ acc (* (saw x) acc 1/2))) [20 500 30])
        )))
