...
(do
  ...
  (l4/control :work :vol {:dur 2048 :to 0.3})
  (l4/control :bass  :vol {:dur 512 :from 0.5 :to 1.0})
  (l4/control :bass2 :vol {:dur 1024 :to 1.0})
  (l4/control :bass3 :vol {:dur 128 :from 1.0 :to 0.1})
   ...
   (map (fn [count ratio]
          (let [gate2 (impulse (* :f4 count))]
            (->  (saw (u/dq (u/throttle :-gate count) (map #(* % 100 ratio)
                                                           [0.1 50 10 1 100])))
                 (* (u/sin-r (* :f8 ratio) 32 1024)
                    (u/dq gate2 [1 1/2 1/4])
                    (env-gen (envelope [0 1 1 0] [0 1e-7 1e-8]) gate2))
                 (u/reduce-> (fn [acc x] (+ acc (* 1/4 (delay-n acc x x)))) [:t2 :t2 :t1])
                 (ringz (u/rg-exp (u/m-map lf-pulse [0.3 0.7 0.8 4.3])
                                  (* ratio 500) (* ratio 2000)) :-ringz-decay)
                 (u/reduce-> (fn [acc x] (free-verb acc x 0.01)) [:-mix1 0.02 0.01])
                 (tanh)
                 (* 2)
                 )))
        [8 4 2 6 16 8]
...
(l4/control :beats :-ringz-decay {:dur 512 :to 0.001})
  ...
  (let [freq (!! (if b3 (* (if (= (mod c0 4) 0) 3 1) 440) 100))]
    (-> (sin-osc (* dr freq))
        ...
        (u/switch-> (u/sin-r (/ 1 dur) 0 1)
                    (ringz (* (u/rg-exp (lf-saw (!! (if b2 1.2 0.7))) 1/4 2) freq) 0.0005))
        tanh)))
...
(defsound drill
  {:period 32
   ...
   :state {:initial (update-state {:count 0})
           :update update-state}}
  (let [freq (!! (if (= (mod c2 4) 0) (+ 500 (rand 1200)) 0))]
    (-> (sin-osc (* dr freq))
        (u/reduce-> (fn [acc [m1 m2]]
                      (sin-osc (lin-exp acc -1 1 (* m1 freq) (* m2 freq))))
                    [[1 2] [1/2 7] [1/4 7]])
        (ringz (* (u/rg-exp (lf-saw (!! (if b0 12 0.1))) 1/4 2) freq) 0.0008)
        tanh)))
...
(defsound bass3
  {:period 16
   ...
   :state {:initial (update-state {:count 0})
           :update update-state}}
  (-> (* (!! (if (= (mod c1 4) 0) 4 2 ))
         (sin-osc 80) (lf-tri (+ (!! (if b0 10 -10)) 50))
